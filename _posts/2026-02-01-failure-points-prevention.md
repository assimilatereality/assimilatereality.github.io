...
Here's something I learned during twenty years managing infrastructure: **every incident you'll handle next year is already built into your systems today**. Not as active bugs waiting to trigger, but as architectural decisions, configuration patterns, and operational practices that create the conditions for future failures.

I've watched a single missing line in a copy-pasted network configuration bring down a global medical system—a change categorized and approved as "low risk, no customer impact" that took hours to identify and propagate fixes for. I've seen service accounts deleted without coordination, causing outages across multiple systems. I've witnessed 39 code changes rolled out together, with hours wasted identifying which one caused the problem buried in the middle.

The pattern is consistent: these weren't mysterious failures that appeared from nowhere. They were predictable consequences of how systems were designed, implemented, and maintained. The medical system went down because the implementation had a single point of failure in the network configuration. The service account deletions caused outages because the company lacked dependency mapping. The problematic code change was hard to identify because a contracted company bundled deployments without proper isolation.
...
At one health insurance system, we discovered "redundant" authentication infrastructure that had a periodic SPOF: both authentication servers, while physically separate, ran on virtual machines sometimes hosted by the same hypervisor as VMs were automatically moved. When that hypervisor experienced issues, both authentication systems became unavailable simultaneously. The redundancy was illusory—the team built in the failure during virtualization planning.

**Insufficient Capacity**

Systems designed to handle expected peak load with minimal buffer guarantee eventual capacity failures. I've watched engineers optimize resource utilization to 90% during normal operations, believing this demonstrated efficiency. Then traffic patterns shifted slightly, or a minor component degradation occurred, and suddenly the system was at 120% capacity with nowhere to go.

The failure was built in as policy optimized for cost over resilience, failing to model realistic growth patterns or account for degraded operation scenarios.
...
That global medical system outage from a missing configuration line? It happened because our network configuration management lacked automated validation. Engineers could copy-paste configurations without verification that all required lines were present.

The failure was built in when the network team prioritized speed of changes over validation rigor. There were no automated checks verifying configuration completeness, no templates that enforced required elements, no peer review requirement for network changes submitted as low risk, no impact because they had performed similar changes hundreds of times.

**Undocumented Dependencies**

The service account deletions that caused cascading outages happened due to no comprehensive dependency mapping. Systems relied on service accounts, but those relationships weren't documented. When someone cleaned up "unused" accounts, they had no way to know what would break and did not check, excusing it as being directed by management.

This failure point was built in when systems were allowed to be implemented without documenting their external dependencies. There was no requirement that new services register their authentication dependencies, no automated discovery of service account usage, no governance around service account lifecycle management.

**Insufficient Testing Coverage**

That code change buried in 39 simultaneous deployments was hard to isolate because vendor testing and deployment practices made it impossible to identify causation and the enterprise oversight did not force better procedures. They bundled changes for efficiency without considering how this would complicate troubleshooting when issues emerged.

The failure was built in when changes were optimized for deployment velocity over diagnostic capability. There was no required isolation of changes, no mandated feature flags for gradual rollout, and no monitoring to correlate changes with behavior.
...
When only one or two people understand critical system components, you've built in a knowledge-based single point of failure. I've watched teams struggle through incidents because the person who understood a particular system was unavailable—on vacation, in a meeting, or having left the company. I've seen a terminated employee be the only person who knew a system password, which he claimed to have forgotten. Fortunately it was a weak password and I was able to guess it with some effort. So, also a security issue from two perspectives.
...
Monthly "SPOF hunts" where teams systematically examine infrastructure diagrams, identify single-instance components, document dependencies on shared resources, and verify redundancy claims are real (not illusory like that authentication system).
...
Quarterly, comprehensive load tests that stress systems beyond expected capacity, use distributed tracing to map actual (not documented) dependencies, profile resource utilization patterns, and identify interaction effects between components.
...
**Flow Diagrams**
For an IBM security solution I pieced together information from multiple documents, even a Microsoft integration document, to create a decision making flow chart for various circumstances, together with layered Visio drawings of system component relationships. (A redacted version was requested by and provided to IBM marketing.) For Citrix, I had a flow chart of its complicated system mounted at my desk. The network team had my 30x60 inch DMZ chart, created through interviews with network team members, on their wall. Per them, it was better than anything they had. If you know where a system broken, you are ahead of the game in resolving the incident.
...
Calculate actual cost of downtime—not just lost revenue, but engineering time responding, customer support costs, reputation damage, and opportunity costs. When leadership sees that a two-hour outage costs $500K, investing in prevention becomes rational. In the health industry an outage can risk lives, for example a suicide prevention hotline I've seen go down many times. No amount of CIO or higher stressing of system importance will fix aging infrastructure without funding.

**Demonstrate Competitive Risk:**

In healthcare, I could point to competitors who maintained availability during 3rd party incident periods while we experienced outages. In financial services, regulatory scrutiny of availability becomes a forcing function. Find your organization's pressure points.

**Start Small and Prove Value:**

Rather than proposing comprehensive resilience engineering programs, identify one high-visibility failure point, eliminate it, and publicize the success. Build momentum through demonstrated value. Going after big systems may seem desirable, but you will run into roadblocks if all you have is words.
...
Mandatory review of designs before implementation catches failure points early. The key is giving these boards actual authority to require changes. SAV and SAS documents can easily exceed 100 pages, and may be the most important documentation of the planned design. Drafters should spot many and plan remedies for potential issues during the preparation of them.
...
Using SRE-style error budgets forces explicit trade-offs between feature velocity and reliability, making failure point elimination part of regular prioritization. Management should encourage and support the use of available error budgets.
...
I've had a QA team state they do not do destructive testing. As a result, I once found a system that allowed an administrator account to be logged into if the password field was left blank, thus exposing PHI and PPI information. How long had it been there? The application had been deployed 3 years previous, updated and, therefore, gone through QA testing multiple times, and there was no logging of account usage.
